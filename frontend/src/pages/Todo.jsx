...
export default function Todo() {
  ...
  const addTodo = async (e) => {
    ...
    try {
      setLoading(true)
      const res = await todos.create(newTodo, null)
    ...
    <div className="space-y-6 slide-in-up">
      ...
      <div className="space-y-2">
        {todoList.filter(t => !t.completed).map(todo => (
          <div
            key={todo.id}
            className="flex items-center gap-3 p-4 rounded-lg border-l-4 bg-white border-pink-400 shadow-sm smooth-transition"
          >
            <input
              type="checkbox"
              checked={false}
      ...
      {completedCount > 0 && (
        <div className="space-y-2">
          <button
            ...
            className="w-full flex items-center justify-between p-3 bg-green-50 rounded-lg text-green-700 font-semibold hover:bg-green-100 smooth-transition"
          >
            <span>Completed ({completedCount})</span>
            <svg 
              className={`w-5 h-5 smooth-transition ${showCompleted ? 'rotate-180' : ''}`} 
              fill="none" 
              stroke="currentColor" 
              viewBox="0 0 24 24"
            >
          ...
          {showCompleted && (
            <div className="space-y-2 slide-in-up">
              {todoList.filter(t => t.completed).map(todo => (
                <div
                  key={todo.id}
                  className="flex items-center gap-3 p-4 rounded-lg border-l-4 bg-green-50 border-green-400 smooth-transition"
                >
                  <input
                    type="checkbox"
                    checked={true}
                    onChange={() => toggleTodo(todo.id, todo.completed)}
                    className="w-5 h-5 text-green-500 cursor-pointer rounded"
                  />
